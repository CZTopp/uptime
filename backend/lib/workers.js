      ...
      checks.forEach((check) => {
        ...
        _data.read("checks", check, (err, originalCheckData) => {
          if (!err && originalCheckData) {
            ...
            debug({
              Error: "Could not read data, check may no longer exist.",
              originalCheckData: JSON.stringify(originalCheckData.id),
            })
          }
        })
      })
    } else {
      debug({ Error: "Could not find any checks to execute" })
    }
  })
}
  ...
  _data.update("checks", newCheckData.id, newCheckData, (err) => {
    if (!err) {
      ...
      if (alertWarranted) {
        ...
        debug({
          Message: `Check id: ${newCheckData.id} outcome has not changed, no alert needed`,
        })
      }
    } else {
      debug({ Error: "unable to save updates to one of the checks" })
    }
  })
}
...
workers.alertUserToStatusChange = (newCheckData) => {
  ...
  helpers.sendTwilioSms(newCheckData.userPhone, msg, (err) => {
    if (!err) {
      console.log({
        Success: "User was alerted of status change via sms",
        Message: msg,
      })
    } else {
      debug({
...
workers.log = (
  ...
  _logs.append(logFileName, logString, (err) => {
    if (!err) {
      ...
      debug({ Error: "Logging to file failed", Message: err })
    }
  })
}
          ...
          if (!err) {
            ...
            _logs.truncate(logId, (err) => {
              if (!err) {
                ...
                debug({ Error: "error truncating log file. " })
              }
            })
          } else {
            debug({ Error: "error compressing a file", err })
          }
        })
      })
    } else {
...
//timer to execute log-rotation once per minute
...
workers.init = () => {
  ...
  //call compression loop so logs will compress later
  workers.logRotationLoop()
}
